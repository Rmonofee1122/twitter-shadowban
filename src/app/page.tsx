...
// 動的インポートでアイコンを遅延読み込み
const FaBan = dynamic(
  () => import("react-icons/fa6").then((mod) => ({ default: mod.FaBan })),
  { ssr: false }
);
const FaCheck = dynamic(
  () => import("react-icons/fa6").then((mod) => ({ default: mod.FaCheck })),
  { ssr: false }
);
const FaQuestion = dynamic(
  () => import("react-icons/fa6").then((mod) => ({ default: mod.FaQuestion })),
  { ssr: false }
);
...
const UserInfo = memo(({ user }: { user: any }) => {
  if (!user)
    return <h2 className="text-bold">ユーザーが見つかりませんでした。</h2>;
  if (user.__typename !== "User")
    return <h2 className="text-bold">ユーザーが凍結されています。</h2>;

  return (
    <div className="flex gap-2">
      <Avatar
        size="lg"
        src={user.legacy.profile_image_url_https.replace("_normal.", ".")}
      />
      <div>
...
export default function Home() {
  ...
  const onSubmit: SubmitHandler<z.infer<typeof TestQuerySchema>> = useCallback(
    async (data) => {
      setLoading(true);
      try {
    ...
    <div className="flex flex-col items-center justify-center my-12">
      <div className="flex flex-col items-center justify-center max-w-4xl w-full px-4 gap-8">
        <h1 className={`${montserrat.className} text-4xl`}>
          Is @{watch("screen_name") ? watch("screen_name") : "Username"}
          <br />
          Shadowbanned on X (Twitter)?
        </h1>
          ...
          <CardBody>
            <form className="flex" onSubmit={handleSubmit(onSubmit)}>
              <Input
                ...
                errorMessage={errors.screen_name?.message}
        ...
        <Card className="dark:bg-neutral-800 w-full">
          ...
          <CardBody>
            {results ? (
              results.user ? (
                results.user.__typename === "User" ? (
                  <p>{results.user.legacy.description}</p>
                ) : (
                  <p>
                    ユーザー情報にアクセスできませんでした、ユーザーが凍結されているかもしれません
                  </p>
                )
              ) : (
                <p>
                  ユーザー情報にアクセスできませんでした、ユーザーが見つかりませんでした
                </p>
              )
            ) : (
              <p>
                シャドウバンとは、X (Twitter)
                において、アカウントがロックや凍結されていないにも関わらず、検索結果や返信一覧に表示されなく(ずらく)なる状態のことです。
              </p>
            )}
          </CardBody>
        </Card>
        <Accordion variant="shadow" className="dark:bg-neutral-800 w-full">
          <AccordionItem
            key="1"
            aria-label="Search Suggestion Ban"
          ...
          >
            ...
            <p>
              検索画面において、検索候補から該当のアカウントが表示されなくなります。
            </p>
          </AccordionItem>
          <AccordionItem
            key="2"
            aria-label="Search Ban"
            title="Search Ban"
            startContent={
              results &&
              results.user &&
          ...
          >
            ...
            <p>
              検索結果から、該当のアカウントのツイートやアカウントが表示されなくなります。
            </p>
          </AccordionItem>
          <AccordionItem
            key="3"
            aria-label="Ghost Ban"
            title="Ghost Ban"
            startContent={
              results &&
              results.user &&
          ...
          >
            ...
            <p>
              ポストに対する返信が返信一覧から表示されなくなり、ポスト投稿主への通知もされなくなります。
            </p>
          </AccordionItem>
          <AccordionItem
            key="4"
            aria-label="Reply Deboosting"
          ...
          >
            ...
            <p>
              ポストに対する返信が返信一覧にて、下部に表示されるようになります。
              <br />
              また、「さらに返信を表示する」をタップするまで返信が表示されなくなる場合があります。
            </p>
          </AccordionItem>
        </Accordion>
      </div>
