...
const XProtectIcon = ({ ...props }: React.SVGProps<SVGSVGElement>) => (
...
const XBlueVerifiedIcon = ({ ...props }: React.SVGProps<SVGSVGElement>) => (
...
const XGoldVerifiedIcon = ({ ...props }: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 22 22" aria-label="認証済みアカウント" role="img" {...props}>
    <g>
...
const XGrayVerifiedIcon = ({ ...props }: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 22 22" aria-label="認証済みアカウント" role="img" {...props}>
    <g>
          ...
          <CardHeader>
            {results ? results.user ? results.user.__typename === 'User' ? (
              <div className="flex gap-2">
                <Avatar size="lg" src={results.user.legacy.profile_image_url_https.replace("_normal.", ".")} />
                <div>
                  <div className="flex items-center gap-0.5">
                    <h2 className="text-lg text-bold">{results.user.legacy.name}</h2>
                    {results.user.legacy.protected ? <XProtectIcon className="w-5 h-5" /> : null}
                    {results.user.is_blue_verified && results.user.legacy.verified_type ? (
                      (() => {
                        switch (results.user.legacy.verified_type) {
                          case 'Business':
